...
import copy
from helper.utils import seed_everything
...
import numpy as np
import warnings
from model.nn import LinearRegression
...
def density_query(b, x_embed, n_cluster, train_mask, seed, data, device):
    ...
    cache_path = osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, n_cluster))
...
def budget_density_query(b, x_embed, train_mask, seed, data, device):
    density = torch.load("./preprocessed_data/aax/density_x_{}_{}.pt".format(x_embed.shape[0], b))
...
def budget_density_query2(b, x_embed, train_mask, seed, data, device):
    density = torch.load("./preprocessed_data/aax/density_x_{}_{}.pt".format(x_embed.shape[0], b)).to(device)
    N = x_embed.shape[0]
    ...
    id_sorted = density.argsort(descending=False).to(device)
    density[id_sorted] = percentile

    n_classes = data.y.max().item() + 1
    density2 = torch.load("./preprocessed_data/aax/density_x_{}_{}.pt".format(x_embed.shape[0], n_classes)).to(device)
    if hasattr(data, 'drop_idx'):
...
def pagerank_query(b, train_mask, data, seed):
    ...
    return indices
def get_subgraph_indices(selected_nodes, test_index):
    subgraph_indices = []
    for node_index in selected_nodes:
        indices = torch.nonzero(test_index == node_index).squeeze(dim=1)
        if indices.numel() > 0:
            subgraph_indices.append(indices.item())
    return subgraph_indices
def pagerank_all_query(b, data_all, data, seed):
    seed_everything(seed)
    edges = [(int(i), int(j)) for i, j in zip(data_all.edge_index[0], data_all.edge_index[1])]
    nodes = list(range(data_all.x.shape[0]))
    data_all.g = nx.Graph()
    data_all.g.add_nodes_from(nodes)
    data_all.g.add_edges_from(edges)
    page = torch.tensor(list(pagerank(data_all.g).values()))
    # page[np.where(train_mask == 0)[0]] = 0
    page[np.where(data_all.test_mask == 0)[0]] = 0
    _, indices_origin = torch.topk(page, k=b)
    test_index = torch.nonzero(data_all.test_mask).squeeze()
    indices = get_subgraph_indices(indices_origin, test_index)
    return indices
...
def pagerank_all_query(b, data_all, data, seed):
    ...
    indices = get_subgraph_indices(indices_origin, test_index)
    return indices
def just_gcn_query(b, gcn_info, data, seed):
    seed_everything(seed)
    conf_score = gcn_info['confidence']
    entropy = gcn_info['entropy']
    entropy_scores = (entropy - min(entropy)) / (max(entropy) - min(entropy))
    alpha = 0.5
    # TODO: how to balance the conf and entropy
    # score = - alpha * conf_score
    score = alpha * entropy_scores
    _, indices = torch.topk(score, k=b)
    print("Only gcn: ", indices)
    return indices
    # # TODO: random 选择
    ...
    #
    # # 升序排序
    # data_sorted = np.sort(conf_score)
    #
    # # 确定数据范围
    ...
    #
    # # 划分区间并统计频数
    # bins = np.linspace(data_min, data_max, num=100)
    # freq, _ = np.histogram(data_sorted, bins=bins)
    #
    # # 画出柱状图
    # plt.bar(bins[:-1], freq, width=(bins[1] - bins[0]), align='edge')
    #
    # # 设置坐标轴标签和标题
    # plt.xlabel('Value')
...
def pagerank_with_gcn_query(b, gcn_info, data, seed):
    seed_everything(seed)
    edges = [(int(i), int(j)) for i, j in zip(data.edge_index[0], data.edge_index[1])]
    nodes = list(range(data.x.shape[0]))
    data.g = nx.Graph()
    ...
    entropy = gcn_info['entropy']
    # TODO: how to balance the conf and entropy
    page_score = alpha * page + beta * conf_score - gamma * entropy
    _, indices = torch.topk(page_score, k=b)
    print("pagerank with gcn: ", indices)
    return indices
def gcn_mix_query(b, gcn_info, data, seed):
    seed_everything(seed)
    edges = [(int(i), int(j)) for i, j in zip(data.edge_index[0], data.edge_index[1])]
    nodes = list(range(data.x.shape[0]))
    num_nodes = data.x.shape[0]
    if not osp.exists("./preprocessed_data/aax/km_{}_{}.pt".format(num_nodes, b)):
        aax_dense = compute_norm_aax(data.x, data.edge_index, num_nodes)
        # aax_dense[np.where(train_mask == 0)[0]] = 0
        distmat = euclidean_distances(aax_dense.cpu().numpy())
        km = kmedoids.fasterpam(distmat, b)
    else:
        km = torch.load("./preprocessed_data/aax/km_{}_{}.pt".format(num_nodes, b), map_location='cpu')

    # c = km.fit(aax_dense)
    # indices = c.medoid_indices_
    selected = torch.tensor(np.array(km.medoids, dtype = np.int32))
    select_mask = torch.Tensor([False for i in range(num_nodes)]).bool()
    select_mask[selected] = True
    total_idxs = torch.arange(num_nodes)
    featprop_select = total_idxs[select_mask]
    feat_score = torch.Tensor([1 if i in featprop_select else 0 for i in range(num_nodes)])
    data.g = nx.Graph()
    data.g.add_nodes_from(nodes)
    data.g.add_edges_from(edges)
    page_score = torch.tensor(list(pagerank(data.g).values()))
    conf_score = gcn_info['confidence']
    entropy = gcn_info['entropy']
    # 将置信度和Page分数放缩到（0，1）
    scaled_confidence = conf_score
    page_scores = (page_score - min(page_score)) / (max(page_score) - min(page_score))
    feat_scores = (feat_score - min(feat_score)) / (max(feat_score) - min(feat_score))
    scaled_scores = page_scores + feat_scores
    # 按照节点的置信度升序排列
    sorted_indices = sorted(range(len(scaled_confidence)), key=lambda x: scaled_confidence[x])
    sorted_confidence = [scaled_confidence[i] for i in sorted_indices]

    # 获取排序后第b个节点的置信度
    threshold_confidence = sorted_confidence[b - 1] + 0.1

    # 根据置信度阈值选择节点
    # selected_nodes = [sorted_indices[i] for i, confidence in enumerate(scaled_confidence) if confidence < threshold_confidence]
    selected_nodes_conf = [i for i, confidence in enumerate(scaled_confidence) if confidence < threshold_confidence]
    # 在选择的节点范围内根据Page分数选择前b个节点
    selected_nodes_scores = [scaled_scores[i] for i in selected_nodes_conf]
    sorted_scores_indices = sorted(range(len(selected_nodes_scores)), key=lambda x: selected_nodes_scores[x],
                                   reverse=True)
    selected_nodes = sorted_scores_indices[:b]
    indices = [selected_nodes_conf[i] for i in selected_nodes]
    print("pagerank with gcn: ", indices)
    return indices
def compute_pagerank(data, num_nodes, device):
    if osp.exists("./preprocessed_data/aax/page_{}.pt".format(num_nodes)):
        page = torch.load("./preprocessed_data/aax/page_{}.pt".format(num_nodes))
        return page
    else:
        edges = [(int(i), int(j)) for i, j in zip(data.edge_index[0], data.edge_index[1])]
        nodes = list(range(data.x.shape[0]))
        ...
        torch.save(page, "./preprocessed_data/aax/page_{}.pt".format(num_nodes))
        return page
...
def degree2_query(b, x, data, train_mask, seed, device):
    ...
    density_path = osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, b))
    density = torch.load(density_path, map_location='cuda')
    if hasattr(data, 'drop_idx'):
        density[data.drop_idx] = 0
    # Get percentile
    ...
    id_sorted = density.argsort(descending=False).to(device)
    density[id_sorted] = percentile
    id_sorted = degree.argsort(descending=False).to(device)
...
def degree2_with_gcn_query(b, x, data, gcn_info, seed, device):
    seed_everything(seed)
    num_nodes = x.shape[0]
    N = num_nodes
    ## get aax
    ...
    id_sorted = degree.argsort(descending=False).to(device)
    degree[id_sorted] = percentile
    id_sorted = conf_score.argsort(descending=True).to(device)
    conf_score[id_sorted] = percentile
    id_sorted = entropy.argsort(descending=False).to(device)
    entropy[id_sorted] = percentile
    # Get linear combination
    alpha= data.params['age'][0]
    age_score = alpha * density + (1 - alpha) * degree + 0.5 * conf_score + 0.5 * entropy
    # age_score[np.where(train_mask == 0)[0]] = 0
    _, indices = torch.topk(age_score, k=b)
    return indices
...
def pg2_query(b, x, data, train_mask, seed, device):
    ...
    density_path = osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, b))
    density = torch.load(density_path, map_location='cpu').to(device)
    if hasattr(data, 'drop_idx'):
        density[data.drop_idx] = 0
    # Get percentile
    percentile = (torch.arange(N, dtype=x.dtype, device=device) / N)
    id_sorted = page.argsort(descending=False).to(device)
...
def age_query(b, x, data, train_mask, seed, device):
    ...
    density_path = osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, b))
...
def age_query2(b, x, data, train_mask, seed, device):
    ...
    d_path = osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, num_classes))
...
def cluster2_query(b, x, data, edge_index, train_mask, seed, device):
    ...
    num_nodes = x.shape[0]
    _, density = density_query(b, x, data.y.max().item() + 1, train_mask, seed,data, device)
    if osp.exists("./preprocessed_data/aax/center_{}_{}.pt".format(num_nodes, b)):
        centers = torch.load("./preprocessed_data/aax/center_{}_{}.pt".format(num_nodes, b))
    else:
        ...
        torch.save(centers, "./preprocessed_data/aax/center_{}_{}.pt".format(num_nodes, b))
...
def cluster_query(b, x, edge_index, train_mask, seed, device):
    ...
    aax = compute_norm_aax(x, edge_index, num_nodes)
    # import ipdb; ipdb.set_trace()
    if osp.exists("./preprocessed_data/aax/center_aax_{}_{}.pt".format(num_nodes, b)):
        centers = torch.load("./preprocessed_data/aax/center_aax_{}_{}.pt".format(num_nodes, b), map_location='cpu')
    else:
        kmeans = Cluster(n_clusters=b, n_dim=x.shape[1], seed=seed, device=device)
        kmeans.train(aax.cpu().numpy())
        centers = torch.tensor(kmeans.get_centroids(), dtype=aax.dtype, device=aax.device)
        torch.save(centers, "./preprocessed_data/aax/center_aax_{}_{}.pt".format(num_nodes, b), map_location='cpu')
...
def compute_norm_aax(x, edge_index, num_nodes):
    ...
    # import ipdb; ipdb.set_trace()
    cache_path = osp.join("./preprocessed_data/aax", 'aax_{}.pt'.format(num_nodes))
    if os.path.exists(cache_path):
...
def compute_density_aax(aax, num_nodes, b, device):
    ...
    if osp.exists("./preprocessed_data/aax/density_aax_{}_{}.pt".format(num_nodes, b)):
        aax_density = torch.load("./preprocessed_data/aax/density_aax_{}_{}.pt".format(num_nodes, b), map_location='cpu')
        return aax_density
    else:
        ...
        torch.save(aax_density, "./preprocessed_data/aax/density_aax_{}_{}.pt".format(num_nodes, b))
        return aax_density
...
def partition_hybrid(b, num_centers, data, train_mask, compensation, x_embed, seed, device, max_part):
    ...
    # density_query(b, x_embed, n_cluster, train_mask, seed, data, device):
    _, reliability = density_query(b, x_embed, n_cluster, train_mask, seed, data, device)
    num_parts = int(np.ceil(b / num_centers))
    # compensation = 0
    if num_parts > max_part:
        num_parts = max_part
...
def compute_adj2(edge_index, num_nodes):
    ...
    cache_path = osp.join("./preprocessed_data/aax", 'adj2_{}.pt'.format(num_nodes))
    if os.path.exists(cache_path):
        res = torch.load(cache_path, map_location='cpu')
        return res
    else:
...
def compute_sim(norm_aax, num_nodes):
    print("compute sim")
    cache_path = osp.join("./preprocessed_data/aax", 'sim_{}.pt'.format(num_nodes))
    if os.path.exists(cache_path):
        res = torch.load(cache_path, map_location='cpu')
        return res
    else:
...
def lrim_wrapper(b, edge_index, train_mask, data, oracle_acc, th, batch_size, reliability_list, seed, density_based = False):
    ...
    if density_based:
        density = torch.load("./preprocessed_data/aax/density_x_{}_{}.pt".format(x_embed.shape[0], n_class))
...
def iterative_score(b, edge_index, oracle_acc, train_mask, data, th, reliability_list, batch_size, seed, device):
    ...
    density_score = torch.load("./preprocessed_data/aax/density_x_{}_{}.pt".format(num_nodes, n_clusters))
...
def generate_pl_mask(data, no_val = 0, total_budget = 140, seed = 0):
    seed_everything(seed)
    total_num = data.test_mask.sum().item()
...
#     total_num = data.test_mask.sum().item()
#     total_label_num = total_budget
#     test_label_mask = data.test_mask
