...
class Experiment:
    ...
    def sync_api(self, prompts, temperature = None, top_p = None, n = 1, dataset = 'cora', key = 'zero_shot', rewrite = False):
        ...
        for i, prompt in tqdm(enumerate(prompts)):
            if prompt == "":
                responses.append("")
                # 空prompt对应空response，然后进行下一次迭代
                continue
            # 缓存存在且不为空
            if cache != None and cache[i] != "" and not rewrite:
                responses.append(cache[i]) #直接将缓存中的值添加到 responses 列表中，并跳过后续的 API 调用。
            else:
                if temperature != None:
    ...
    def test_no_correction(self, prompts, label_names, temperature = None, top_p = None, n = 1, dataset = 'cora', key = 'zero_shot', rewrite = False, fix = False, seed = 0, sp = 2, ss = 2):
        ...
        for i, res in tqdm(enumerate(openai_result)):
            if i not in select_ids:
                ...
                continue
            try:
                # 对每个回复进行语法检查和错误修正。如果检查无误，则将回复添加到 responses 列表中，否则将空字符串添加到 responses 中，并将错误计数器加1。
    ...
    # 异步调用API
    # 程序在发送 API 请求后，不会立即等待响应结果，而是继续执行后面的代码。当 API 返回响应结果时，程序会得到通知，并在需要时处理响应结果。
    # 在等待期间，程序可以继续执行其他任务，提高了程序的并发性能。这种方式适用于请求量较大或响应时间较长的场景。
    def async_api(self, prompts, label_names, temperature = None, top_p = None, n = 1, dataset = 'cora', key = 'zero_shot', rewrite = False, fix = False, seed = 0, sp = 2, ss = 2):
        ...
        for i in range(len(prompts)):
            # 函数首先检查缓存是否存在有效结果，并根据需要判断是否使用缓存。缓存命中 → 直接将cache[i]作为响应结果
...
Analysis, answer, and confidence: ".format(question)
    ...
    # 生成一个多步骤分析问题的提示文本
    # 在每个步骤中提供推理和置信度，并在最后给出最终答案和整体置信度
...
".format(question)
    ...
    def topk_confidence(self, question, k = 3, name = 'arxiv'):
        if name == 'arxiv':
            #提示文本要求用户根据给定的论文标题，判断它属于哪个 arXiv 论文的计算机科学子类别，并且按照一定格式输出答案和置信度。
...
".format(question, k)
    ...
    # 处理多个回答的答案 将其格式化为列表的形式
    def retrieve_multiple_answers(self, answer, data, num):
        output = [] # 存储结果
        invalid = 0 # 记录无效的回答数量
        for result in answer:
            # import ipdb; ipdb.set_trace()
            if result == "": # 空答案则将空列表加入当前output，然后跳出循环
                res = [("", 0)]
                output.append(res)
                continue
            res = []
            for r in result: # 回答中的每行
            # import ipdb; ipdb.set_trace()
                line = r.lower() #转变为小写字母
                this_line = [] #存储当前行的处理结果
                try:
                    ## if no error, retrieve all dicts in a list
                    this_dict = retrieve_dict(line) # 解析当前行的字典数据
...
# Give [k] likely arXiv CS sub-categories as a comma-separated list ordered from most to least likely, in the form “cs.XX”, and provide your confidence score.
...
# 零样本学习的提示文本
# texts：包含多个文本的列表，每个文本表示一个对象的内容
# labels_names：是一个包含所有可能的标签名称
# need_tasks：布尔值，是否需要在提示文本中包含任务描述
...
# few_shot_prompts基础上进行修改，生成少样本学习的提示文本
def few_shot_topk_prompts(texts, label_names, example, example_dict, exp, object_cat = "Paper", question = "Which arxiv cs subcategories does this paper belong to?", answer_format = "Give 3 likely arXiv CS sub-categories as a comma-separated list ordered from most to least likely together with a confidence ranging from 0 to 100, in the form of a list python dicts like [{\"answer:\":<answer_here>, \"confidence\": <confidence_here>}]", name = 'arxiv'):
    ...
    for text in texts:
        ...
        in_context += "Task: \n"
        if not 'arxiv' in question:
        # 如果问题描述中不包含 "arxiv" 字符串，将可选的类别名称列表添加到 in_context 中
...
#从给定的提示文本列表中选择指定数量的提示文本
# 根据给定的索引列表，从原始的提示文本列表中选择对应索引的提示文本，并按照顺序放入新的提示文本列表
...
#最常见
#使用给定的文本作为输入，调用指定的API模型生成响应，然后检查响应与给定的标签数据之间的一致性
...
def zero_shot_query(orig_question, data, configs, dataname, select_ids, rewrite = False):
    ...
    # 使用 vanilla_verbalized_confidence 方法生成基准提示文本
    vanilla_prompt = [exp.vanilla_verbalized_confidence(q) for q in questions]
    # 对提示文本进行处理，根据选择的ID和数据总数进行打包
...
# 主要用于测试不同模型在指定数据集上的表现，并生成相应的结果报告
if __name__ == '__main__':
    ...
    for dataname in need_datasets:
        ...
        for seed in seeds:
            ...
            #zero_shot时：通过原始提示文本，使用模型生成回答，评估生成回答的性能和成本
            ...
            # 区别：使用test_no_correction()函数而不是异步API调用来生成回答
            if 'no_fix' in exps:
                print("No fix")
                vanilla_prompt = [exp.vanilla_verbalized_confidence(q) for q in questions]
                prompt_cost = calculate_cost_from_a_list_of_texts(vanilla_prompt, select_ids)
            ...
            # 将所有可用的示例都用于训练策略，提供更多信息，但有可能过拟合
            #区别在于调用few_shot_topk_prompts()函数
            ...
            # 从每个数据集中选择很少的示例来进行训练，在很少的示例情况下学会泛化
            # 区别在于调用few_shot_prompts()函数
            ...
            if 'topk' in exps:
                print("topk")
                # 生成一组用于提供top-k预测的提示文本topk_step_prompt
