...
def main(data_path, args = None, custom_args = None, save_best = False):
    ...
    filename = f"D:/code/LLM_data/concept_degree/{args.dataset}_{args.strategy}_{args.second_filter}_b{budget}.pt"
    ...
    if os.path.isfile(filename):
        ...
        train_masks, val_masks, test_masks = active_selection(args, test_data, llm_pred[test_mask], conf[test_mask],
                                                              args.split,
    ...
    return np.var(results_ttt), np.mean(results_ttt)
...
if __name__ == '__main__':
    ...
    datasets = ['cora', 'pubmed', 'citeseer', 'wikics']
    if args.mode == "main":
        for db in datasets:
            if db == 'cora':
                budgets = [83]
        ...
        for db in datasets:
            ...
            for b in budgets:
                ...
                study.optimize(sweep, n_trials=1)
                #df = study.trials_dataframe(attrs =('number', 'value', 'params', 'state'))
                df = study.trials_dataframe()
                filename = f"D:/code/LLM/LLMTTT/some TTT results/all_data/concept_degree/{db}_{b}_{args.strategy}_{args.second_filter}_{args.filter_strategy}_wo_stage1.csv"
                df.to_csv(filename, index=False)
                df = pd.read_csv(filename)
                df.to_excel(f"D:/code/LLM/LLMTTT/all_data/concept_degree/{db}_{b}_{args.strategy}_{args.second_filter}_{args.filter_strategy}_wo_stage1.xlsx", index=False)
                print(df)
