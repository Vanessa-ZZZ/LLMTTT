import faiss
from helper.data import get_dataset, load_ood
from helper.active import compute_norm_aax
from helper.args import get_command_line_args
from helper.utils import load_yaml, extract_subgraph
import torch
import ipdb
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        if dataset  == 'cora':
            ...
            budgets = [5148]
        reliability_list = []
        # data = get_dataset(seeds, dataset, args.split, args.data_format, params_dict['DATA_PATH'], None, 0, args.no_val, args.budget,args.strategy, args.num_centers, args.compensation, args.save_data, args.filter_strategy, args.max_part, 1 - args.pl_noise, reliability_list, args.total_budget, 'none', False)
        data, _, _, _ = load_ood(dataset, data_path, 'GOOD')
        test_data = extract_subgraph(data, data.test_mask)
        data = extract_subgraph(data, data.test_mask)
        x = data.x.detach()
        y = data.y
        ...
        torch.save(centers_1, osp.join("./preprocessed_data/aax", 'center_x_{}_{}.pt'.format(num_nodes, cluster_1)))
        torch.save(centers_2, osp.join("./preprocessed_data/aax", 'center_x_{}_{}.pt'.format(num_nodes, cluster_2)))
        # torch.save(centers_3, osp.join("./preprocessed_data/aax", 'center_x_{}_{}.pt'.format(num_nodes, cluster_3)))
        # torch.save(centers_4, osp.join("./preprocessed_data/aax", 'center_x_{}_{}.pt'.format(num_nodes, cluster_4)))
        # torch.save(centers_5, osp.join("./preprocessed_data/aax", 'center_x_{}_{}.pt'.format(num_nodes, cluster_5)))
        ## save x density
        ...
        torch.save(density_1, osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, cluster_1)))
        torch.save(density_2, osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, cluster_2)))
        # torch.save(density_3, osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, cluster_3)))
        # torch.save(density_4, osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, cluster_4)))
        # torch.save(density_5, osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, cluster_5)))
        # torch.save(density_6, osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, cluster_6)))
        # torch.save(density_7, osp.join("./preprocessed_data/aax", 'density_x_{}_{}.pt'.format(num_nodes, cluster_7)))
        ...
        # centers_6 = torch.tensor(centers_6, dtype=x.dtype, device=x.device)
        # centers_7 = torch.tensor(centers_7, dtype=x.dtype, device=x.device)
        torch.save(centers_1, osp.join("./preprocessed_data/aax", 'center_aax_{}_{}.pt'.format(num_nodes, cluster_1)))
        torch.save(centers_2, osp.join("./preprocessed_data/aax", 'center_aax_{}_{}.pt'.format(num_nodes, cluster_2)))
        # torch.save(centers_3, osp.join("./preprocessed_data/aax", 'center_aax_{}_{}.pt'.format(num_nodes, cluster_3)))
        # torch.save(centers_4, osp.join("./preprocessed_data/aax", 'center_aax_{}_{}.pt'.format(num_nodes, cluster_4)))
        # torch.save(centers_5, osp.join("./preprocessed_data/aax", 'center_aax_{}_{}.pt'.format(num_nodes, cluster_5)))
        # torch.save(centers_6, osp.join("./preprocessed_data/aax", 'center_aax_{}_{}.pt'.format(num_nodes, cluster_6)))
        # torch.save(centers_7, osp.join("./preprocessed_data/aax", 'center_aax_{}_{}.pt'.format(num_nodes, cluster_7)))
        ...
        # dist_map_7 = torch.tensor(dist_map_7, dtype=x.dtype, device=x.device)
        # density_7 = 1 / (1 + dist_map_7)
        torch.save(density_1, osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, cluster_1)))
        torch.save(density_2, osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, cluster_2)))
        # torch.save(density_3, osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, cluster_3)))
        # torch.save(density_4, osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, cluster_4)))
        # torch.save(density_5, osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, cluster_5)))
        # torch.save(density_6, osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, cluster_6)))
        # torch.save(density_7, osp.join("./preprocessed_data/aax", 'density_aax_{}_{}.pt'.format(num_nodes, cluster_7)))
        # import ipdb; ipdb.set_trace()